...
const uint32_t mask_clear = (((HS8_Pin|HS4_Pin|HS2_Pin|HS1_Pin)|(LSH10_Pin)|(LSH01_Pin)|(LSM10_Pin)|(LSM01_Pin))<<16);
uint32_t Column[] = {
		mask_for_pins|(LSH10_Pin<<16)|(LSH01_Pin)|(LSM10_Pin)|(LSM01_Pin),// H10 Column
...
void STM32L4_BCDDisplayManager::show()
{
	...
	switch (request.getType()) {
		...
		case SetCorrMonth:
			...
			DisplayBuffer[0]=numToPort[request[0]%10];
			...
			break;
		case SetTempCorr:
			DisplayBuffer[2]=numToPort[request[0]/100];
			DisplayBuffer[1]=numToPort[(request[0]/10)%10];
			DisplayBuffer[0]=numToPort[request[0]%10];
		...
		case ShowStepsHistory:
		{
				...
				DisplayBuffer[1]=numToPort[((steps)/10)%10];
				DisplayBuffer[0]=numToPort[((steps)%10)];
				DisplayBuffer[3]=DISP_4|numToPort[request[0]%4];
				break;
		}
		case ShowUBatt:
		{
